...
namespace Network.IPAddress
{
	...
	{
		#region 私有成员
		private const string InvalidIP="IP格式错误";
		...
		private int QQwry()
		{
			...
			if(!objMa.Success)
			{
				try
				{
					System.Net.IPHostEntry ipEntry =System.Net.Dns.GetHostByName(ip);
					if (ipEntry.AddressList.GetLength(0)>0)
						ip=ipEntry.AddressList[0].ToString();
					else
					{
						this.errMsg=InvalidIP;
						return 4;
					}
				}
				catch
